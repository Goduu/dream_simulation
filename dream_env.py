...
class DreamEnv:
    ...
    def observe(self):
        ...
        buy_possibilities = self.reward_store.get_possible_buys(self.current_player)
    ...
    def step(self, action):
        ...
        self.current_player.check_termination(mov_possibilities,buy_possibilities)
        ...
        if(not self.current_player.terminated):
            ...
            if(action_type == "mov"):
                ...
                print("BUUUUY REWARD", reward_to_buy)
                self.reward_store.buy_reward(self.current_player,reward_to_buy)

        end_round = self.is_end_round()
        if(end_round):
            self.account_resources()
            if(self.round < self.rounds ):
                print("FINISHING ROUND", self.round)
                self.next_round()
            else: 
